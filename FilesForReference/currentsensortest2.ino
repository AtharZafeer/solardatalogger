...
float cutOffLimit = .01;// set the current which below that value, doesn't matter. Or set 0.5
...
void loop() {
  ...
  //Robojax.com ACS712 Current Sensor
  float voltage_raw =   (5.0 / 4095.0)* analogRead(VIN);// Read the voltage from sensor
  ...
  if(abs(current) > cutOffLimit ){
    Serial.print("V: ");
    Serial.print(voltage_raw,3);// print voltage with 3 decimal places
    Serial.print("V, I: ");
